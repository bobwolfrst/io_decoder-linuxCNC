...
<div class="nav-container">
  ...
  <a href="{{ '/demo_mode.it' | relative_url }}" class="nav-btn lang-btn">
...
### 3. HAL Configuration
  ...
  #### Synopsis
   ...
   - **usb_port_name**: Allows you to name the port as desired for persistence; see the [**Define USB Port**]({{ '/README.en#installation-commands' | relative_url }}) section. Default value: `"/dev/io_decoder"`.
   ...
   - **verbose**: Enables the level of error messages displayed on the GUI. The selected number activates that message type and all lower-value levels. Default: `1`.  
     ...
     - `3` = All.
   - **keymap_file**: Text file to set the [**input => keyboard simulation**]({{ '/README.en#keyboard' | relative_url }}) mappings. Default value: `"io_decoder-keymap.cfg"`.
   - **uinput_chmod_cmd**: String parameter to grant write permissions on UINPUT for the [**simulated keyboard functionality**]({{ '/README.en#keyboard' | relative_url }})... 
...
### 4. Testing and Diagnostics
  ...
  In the Pins section, look for:
   ...
   [Full list of available HAL pins]({{ '/README.en#pins' | relative_url }}).
